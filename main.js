...
async function updateTwoMovieDetails() {
    ...
    while (foundMovies < 2 && attempts < movieTitles.length) {
        ...
        if (!selectedMovies.includes(title)) {
            ...
            if (movieDetails && movieDetails.Response === "True" && hasRequiredDetails(movieDetails)) {
    ...
    if (foundMovies < 2) {
        throw new Error("No valid films with sufficient detail were found after several attempts.");
    }
}
